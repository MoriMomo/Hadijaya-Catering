...
## 2024-06-18 - Repeated Button Labels Lacking Context
**Learning:** Found that repeated "Tambah ke Pesanan" buttons in a list or grid lack context for screen reader users when read out of sequence. Screen readers read "Tambah ke Pesanan" repeatedly without knowing which item it applies to.
**Action:** Always provide an item-specific `aria-label` (e.g., `aria-label={"Tambah " + item.name + " ke pesanan"}`) for repeated action buttons to give complete context to assistive technologies.
