...
## 2024-07-26 - Contextual Labels on Repeated Buttons
**Learning:** Having multiple buttons with the same generic text like "Tambah ke Pesanan" (Add to Order) or "Lihat Semua" (See All) across a grid creates a frustrating experience for screen reader users, who lack visual context of which item the button relates to.
**Action:** Always provide an item-specific `aria-label` (e.g., `aria-label={"Tambah " + item.name + " ke pesanan"}`) on repeated action buttons to give complete context when navigating via accessibility trees.
